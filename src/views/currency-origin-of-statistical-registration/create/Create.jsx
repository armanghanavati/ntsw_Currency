// صفحه ایجاد منشا ارز ثبت آماری
import {
  handleLoading,
  handleMessageModal,
} from "../../../state/action-creators";
import { endpoints } from "../../../services/endpoints";
import axios from "axios";
import { useDispatch, useSelector } from "react-redux";
import { useEffect } from "react";
import { useState } from "react";
import {
  useHistory,
  useLocation,
} from "react-router-dom/cjs/react-router-dom.min";
import { Form } from "antd";
import {
  CurrencyOriginsTable,
  DataForAddItemToCurrencyOriginsTable,
  AddingBillOfLadingByUser,
} from "../../../common";
import { Button, VerticalSpace } from "../../../components";
import Inquiry from "./common/Inquiry";
import themeColors from "../../../configs/theme";
import SubmitData from "./common/SubmitData";
import currencyOperationTypes from "../../../enums/currency-operation-types";

const Create = () => {
  // دو لیست از نقش هایی که منوی منشا ارز ثبت آماری برای ان ها نمایش داده می‌شود

  // بر اساس این لیست سرویس مربوط به استعلام کال میشه
  const roleTypeCheckList1 = [101, 9, 105];
  // براساس این لیست اپشن های نوع منشا ارز متفاوت خواهد بود
  const roleTypeCheckList2 = [105, 9, 101];

  const dispatch = useDispatch();
  const { search, state } = useLocation();
  const { role, GUid } = useSelector((state) => state);

  const [form] = Form.useForm();
  const history = useHistory();

  const [customsOptions, setCustomsOptions] = useState([]);
  const [currencyOriginOptions, setCurrencyOriginOptions] = useState([]);
  const [currencyOptions, setCurrencyOptions] = useState([]);
  const [prfVCodeInt, setPrfVCodeInt] = useState();
  const [prfOrderNoStr, setPrfOrderNoStr] = useState();
  const [inputsData, setInputsData] = useState({});
  const [
    tableDataSourceForCurrencyOrigins,
    setTableDataSourceForCurrencyOrigins,
  ] = useState([]);
  const [tableDataSourceForBillOfLading, setTableDataSourceForBillOfLading] =
    useState([]);
  const [errors, setErrors] = useState({});

  const getProformaInfoForCreatePreCotage = (prfVCodeInt) => {
    dispatch(handleLoading(true));
    const postData = {
      urlVCodeInt: role,
      ssdsshGUID: GUid,
      prfVCodeInt,
    };
    axios({
      url: endpoints.RestAPIs.preCotageSG.getProformaInfoForCreatePreCotage.url,
      method:
        endpoints.RestAPIs.preCotageSG.getProformaInfoForCreatePreCotage.method,
      data: postData,
    })
      .then((res) => {
        dispatch(handleLoading(false));
        if (res?.data?.ErrorCode === 0) {
          setInputsData({
            prfOrderNoStr: res?.data?.prfOrderNoStr,
            curNameStr: res?.data?.curNameStr,
            prfTotalPriceMny: res?.data?.prfTotalPriceMny,
          });
        } else {
          dispatch(
            handleMessageModal({
              isModalOpen: true,
              describe: res?.data?.ErrorDesc,
            })
          );
        }
      })
      .catch((err) => {
        console.log(err);
        dispatch(handleLoading(false));
      });
  };

  useEffect(() => {
    if (state?.hasAccess) {
      const key = atob(
        new URLSearchParams(search).get("key").replace(/=/g, "")
      );
      const key2 = atob(
        new URLSearchParams(search).get("key2").replace(/=/g, "")
      );
      setPrfOrderNoStr(+key);
      setPrfVCodeInt(+key2);
      getProformaInfoForCreatePreCotage(+key2);
    } else {
      dispatch(
        handleMessageModal({
          isModalOpen: true,
          describe:
            "شما به این صفحه دسترسی ندارید. لطفا از طریق دکمه ایجاد منشا ارز جدید وارد این صفحه شوید.",
        })
      );
      handleReturn();
    }
  }, []);

  const handleValidation = (inputsName = []) => {
    const err = { ...errors };
    inputsName.map((item) => {
      if (
        (inputsData[item] === 0 ||
          inputsData[item] === undefined ||
          inputsData[item] === null ||
          JSON.stringify(inputsData[item])?.trim() === "") &&
        item !== undefined
      ) {
        err[item] = ["پرکردن این فیلد الزامی است"];
      }
    });
    setErrors(err);
    return err;
  };

  const permitForSkip = (inputsName = []) => {
    const error = handleValidation(inputsName);
    for (var key in error) {
      if (error[key]?.length > 0) {
        if (inputsName?.includes(key)) {
          return false;
        }
      }
    }
    return true;
  };

  const handleReturn = () => {
    history.push("/Users/AC/Currency/CurrencyOriginOfStatisticalRegistration");
  };

  return (
    <>
      {!!inputsData?.prfOrderNoStr && (
        <>
          <DataForAddItemToCurrencyOriginsTable
            inputsData={inputsData}
            setInputsData={setInputsData}
            errors={errors}
            setErrors={setErrors}
            prfVCodeInt={prfVCodeInt}
            form={form}
            currencyOperationType={
              currencyOperationTypes.StatisticalRegistration
            }
            currencyOriginOptions={currencyOriginOptions}
            setCurrencyOriginOptions={setCurrencyOriginOptions}
            customsOptions={customsOptions}
            setCustomsOptions={setCustomsOptions}
            roleTypeCheckList2={roleTypeCheckList2}
            currencyOptions={currencyOptions}
            setCurrencyOptions={setCurrencyOptions}
          />
          <VerticalSpace space="10px" />
          <Inquiry
            prfOrderNoStr={prfOrderNoStr}
            inputsData={inputsData}
            setInputsData={setInputsData}
            setErrors={setErrors}
            permitForSkip={permitForSkip}
            setTableDataSourceForCurrencyOrigins={
              setTableDataSourceForCurrencyOrigins
            }
            form={form}
            roleTypeCheckList1={roleTypeCheckList1}
            currencyOriginOptions={currencyOriginOptions}
            customsOptions={customsOptions}
            currencyOptions={currencyOptions}
          />
          <VerticalSpace space="10px" />
          <CurrencyOriginsTable
            tabledataSource={tableDataSourceForCurrencyOrigins}
            setTableDataSource={setTableDataSourceForCurrencyOrigins}
            currencyOperationType={
              currencyOperationTypes.StatisticalRegistration
            }
          />
          <VerticalSpace space="10px" />
          <AddingBillOfLadingByUser
            tabledataSource={tableDataSourceForBillOfLading}
            setTabledataSource={setTableDataSourceForBillOfLading}
          />
          <div className="steps-action">
            <Button
              onClick={handleReturn}
              backgroundColor={themeColors.btn.danger}
            >
              <i class="fa fa-share" aria-hidden="true"></i>
              بازگشت
            </Button>
            <SubmitData
              tableDataSourceForBillOfLading={tableDataSourceForBillOfLading}
              tableDataSourceForCurrencyOrigins={
                tableDataSourceForCurrencyOrigins
              }
              prfVCodeInt={prfVCodeInt}
            />
          </div>
        </>
      )}
    </>
  );
};

export default Create;
